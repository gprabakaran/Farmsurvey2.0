function myapp(client, userId) {
    ...
    app.FarmDwelling = Backbone.Force.Model.extend({
        ...
        incompleteFields1: function () {
            var missingFields1 = new Array();
            for (var i = 0; i < this.required.length; i++) {
                if (this.get(this.required[i]) === 'test') {
                    missingFields1.push(this.required[i]);
                }
            }
            return missingFields1;
        },
